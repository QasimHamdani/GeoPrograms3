public class B3_HalfTheRed extends World {
    ...
    public void go() {
        ...
        for (int row = 1; row < 793; row = row + 1) {
            for (int col = 1; col < 560; col++) {
                ...
                if (red > 10 && red < 210) {
                    plane.setPixelColor(col / 4, 13, 100);
                    //if (blue > 30 && red < 150 && green > 25 && row > 1 && col > 5) {
                    // if (green < 45 && col < 570) {
                }
        ...
        for (int row = 1; row < 793; row = row + 1) {
            for (int col = 1; col < 560; col++) {
                ...
                if (red > 200 && red < 245 && blue > 200 && green > 200) {
                    plane.setPixelColor(col / 4, green, row / 4);
//                plane.teleport(col, 488 - row);
                    //plane.setPixelColor(row / 2, green, blue);
                }
            }
        }
        for (int row = 1; row < 793; row = row + 1) {
            for (int col = 1; col < 560; col++) {
                ...
                if (red > 215 && red < 245 && blue < 70 && green < 200) {
                    plane.setPixelColor(120, 1, 170);
                }
            }
        }
    }
